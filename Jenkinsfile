pipeline {
    ...
    stages {
        stage('Cloner le dépôt Git') {
            steps {
                git 'https://github.com/zhengsunny2/jenkins-Selenium-test.git'  // Remplacez par votre dépôt GitHub
            }
        }
        ...
        stage('Déployer en environnement de test') {
            steps {
                script {
                    ...
                    docker.image(DOCKER_IMAGE).run('-d -p 3000:3000')  // Exécuter l'application en arrière-plan
                }
            }
        }
        ...
        stage('Vérification de l\'application en environnement de test') {
            steps {
                script {
                    ...
                    sh 'curl http://localhost:3000' // Ou exécuter d'autres tests de validation
                }
            }
        }
        ...
        stage('Déployer en production') {
            ...
            steps {
                script {
                    ...
                    docker.image(DOCKER_IMAGE).run('-d -p 8080:3000')  // Exécuter l'application sur un autre port pour la production
                }
            }
        }
    }
